...
int main(int argc, char* argv[]) {
  ...
  for (i=0; i<NumLam; i++) {
     ...
     // Pure W underlayer
     MaxwellGarnett(vf1, epsbg, w_sub[i], &eta, &kappa);

     rind[0] = 1. + 0.*I;    
     n1 = creal(rind[0]);
     //rind[1] = eta+kappa*I; 
     //rind[2] = 1.75+0.*I;  //tio2_rind[i];
     //rind[2] = alumina_ald[i];  
     ...
     rho1 = Fac*(2*h*c*c/pow(lambda,5))*(1/(exp(h*c/(lambda*kb*T1))-1));
     rho2 =     (2*h*c*c/pow(lambda,5))*(1/(exp(h*c/(lambda*kb*T2))-1));
