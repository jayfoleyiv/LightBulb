...
double SpectralEfficiency(double *emissivity, double thetaI, int N, double *lambda, double lambdabg, double Temperature, double *P);
...
int main(int argc, char* argv[]) {
  ...
  for (i=0; i<NumLam; i++) {
     ...
     rho = (2*h*c*c/pow(lambda,5))*(1/(exp(h*c/(lambda*kb*Temp))-1))*cos(thetaI);
  ...
  double SE = SpectralEfficiency(Emiss, thetaI, NumLam, LamList, lbg, Temp, &PU);
...
double SpectralEfficiency(double *emissivity, double thetaI, int N, double *lambda, double lbg, double T, double *P){
    int i;
    ...
    for (i=1; i<N-1; i++) {
      ...
      em = emissivity[i];
      l = lambda[i];
      rho = (2*pi*h*c*c/pow(l,5))*(1/(exp(h*c/(l*kb*T))-1))*cos(thetaI);
